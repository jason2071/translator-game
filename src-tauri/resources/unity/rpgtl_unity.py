...
def _apply_tables(t, placed, keep_gidx):
    ...
    # NOTE: Thai stacks marks two-high above the base and hangs vowels below it, so in a
    # font whose line box was drawn tight for CJK/Latin (e.g. 851tegaki ships
    # LineHeight == PointSize) multi-line Thai crowds line-to-line. We deliberately do NOT
    # grow the face's line height to compensate: on a fixed, auto-sizing text box (item
    # descriptions, bios) a taller line height just makes TMP shrink the whole block to
    # fit, which is worse than the mild crowding. There is no single value that both avoids
    # overlap and avoids shrink for a box that tight — it's an inherent CJK→Thai layout
    # limit, best solved per-game in the UI, not by inflating every font's metrics.
