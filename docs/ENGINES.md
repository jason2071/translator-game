...
| **Unity** | managed `TextAsset`s / Addressables bundles / `.assets` (Naninovel, CSV localization, custom `TextTable`, TMPro) | binary | ⚫ Out of scope as files — engines for three Unity storage schemes shipped and were **removed** (the font path needed a 400 MB Python sidecar and still shipped broken). Translate it through XUnity.AutoTranslator instead: ✅ its English cache is supported by the `xunity` engine |
| **XUnity.AutoTranslator English cache** | `BepInEx/Translation/en/Text/_AutoGeneratedTranslations.txt` (`original=translation`) | text | ✅ Supported for BepInEx Mono and IL2CPP — the app translates the original text left of `=` and writes Thai on the right |
...
- **TyranoScript** — `.ks` KAG scenario scripts, either loose under
  ...
  re-export and restore flows. Extracts message text, `[glink text=]`
...
- **Lucky Live** — custom Electron/web game whose story content is stored as loose
  `resources/gioco/content/girls/*/girl.json` files (the Electron `app.asar` is
  only the shell). Extracts character profile text, scene dialogue, chat and
  donation messages, captions, secret hints, and event labels; skips IDs, media
